...
func (s *Stream) packetize(raw []byte, ppi PayloadProtocolIdentifier) ([]*chunkPayloadData, bool) {
	...
	var ownedUserData []byte
	var chunkStorage []chunkPayloadData
	var chunks []*chunkPayloadData
	if remaining != 0 {
		// WriteSCTP must retain an owned copy until every fragment is
		// acknowledged, because callers may reuse raw as soon as WriteSCTP
		// returns. Copy the message once and let its chunks reference disjoint
		// slices of that owned backing array. This preserves the ownership
		// contract while avoiding one allocation and copy operation per SCTP
		// fragment.
		ownedUserData = append([]byte(nil), raw...)
		fragmentCount := int((remaining-1)/s.association.maxPayloadSize + 1)
		chunkStorage = make([]chunkPayloadData, fragmentCount)
		chunks = make([]*chunkPayloadData, fragmentCount)
	}
	...
	for i := 0; remaining != 0; i++ {
		...
		chunk := &chunkStorage[i]
		*chunk = chunkPayloadData{
			streamIdentifier:       s.streamIdentifier,
			userData:               ownedUserData[offset : offset+fragmentSize],
			unordered:              unordered,
