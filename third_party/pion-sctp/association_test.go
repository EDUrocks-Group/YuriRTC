...
func (c *dumbConn2) inboundHandler(packet []byte) {
	...
	if !c.closed {
		// dumbConn2 queues the packet after Write returns, so it must copy the
		// caller-owned slice just like a real net.Conn implementation. Retaining
		// packet here violates io.Writer's ownership contract and races with the
		// association's bounded outbound packet-buffer pool.
		ownedPacket := append([]byte(nil), packet...)
		c.packets = append(c.packets, ownedPacket)
		c.cond.Signal()
	}
}
