...
const (
	packetHeaderSize          = 12
	packetBufferPoolSize      = 1024
	maxPooledPacketBufferSize = receiveMTU * 2
)

// packetBufferPool is bounded globally rather than per association so idle
// peers cannot each retain a private pool. net.Conn.Write follows io.Writer's
// rule that implementations must not retain the supplied slice, so the
// association write loop can safely return a packet after Write completes.
var packetBufferPool = make(chan []byte, packetBufferPoolSize) //nolint:gochecknoglobals

func acquirePacketBuffer(minCapacity int) []byte {
	select {
	case raw := <-packetBufferPool:
		if cap(raw) >= minCapacity {
			return raw[:0]
		}
	default:
	}
...
func releasePacketBuffer(raw []byte) {
	if cap(raw) > int(maxPooledPacketBufferSize) {
		return
	}

	select {
	case packetBufferPool <- raw[:0]:
	default:
	}
}
...
func (p *packet) marshal(doChecksum bool) ([]byte, error) {
	return p.marshalAppend(make([]byte, 0, initialMTU), doChecksum)
}

// marshalAppend serializes into caller-owned storage. Association writes use a
// single packet backing array, avoiding the DATA payload and chunk-header
// allocations from the generic chunk marshal path.
func (p *packet) marshalAppend(raw []byte, doChecksum bool) ([]byte, error) {
	raw = append(raw, make([]byte, packetHeaderSize)...)
	...
	for _, c := range p.chunks {
		if payload, ok := c.(*chunkPayloadData); ok {
			var err error
			raw, err = payload.marshalAppend(raw)
			if err != nil {
				return nil, err
			}
		} else {
		...
		if paddingNeeded != 0 {
			var padding [3]byte
			raw = append(raw, padding[:paddingNeeded]...)
		}
	}
