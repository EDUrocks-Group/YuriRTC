...
type Config struct {
	...
	CwndCAStep uint32
	// CongestionControl selects the sender congestion-control algorithm.
	// The zero value retains upstream Reno behavior.
...
func createAssociationFromConfigWithTsn(cfg *Config, tsn uint32) *Association {
	...
	assoc := &Association{
		...
		congestionController:      newCongestionController(cfg.CongestionControl, cfg.CwndCAStep),
...
func (a *Association) marshalPacket(p *packet) ([]byte, error) {
	raw := acquirePacketBuffer(int(a.MTU()))
	marshaled, err := p.marshalAppend(raw, !a.sendZeroChecksum || chunkMandatoryChecksum(p.chunks))
	if err != nil {
		releasePacketBuffer(raw)
...
// The caller should hold the lock.
func (a *Association) onCumulativeTSNAckPointAdvanced(
	totalBytesAcked int,
	flightSizeBeforeAck uint32,
) {
	// RFC 4960, sec 6.3.2.  Retransmission Timer Rules
	...
	fullyUtilized := controllerWindowWasFullyUtilized(
	...
	if a.CWND() <= a.ssthresh { //nolint:nestif
		...
		//      path MTU.
		if !a.inFastRecovery && fullyUtilized {
		...
		// RFC 9438 replaces Reno's fixed linear increase during congestion
		// avoidance. The controller retains Reno as an explicit fallback.
		acked := boundedIntToUint32(totalBytesAcked)
		srtt := time.Duration(a.SRTT() * float64(time.Millisecond))
		cwnd, partialBytesAcked := a.congestionController.onAck(
			time.Now(),
			a.CWND(),
			a.MTU(),
			acked,
		...
		a.partialBytesAcked = partialBytesAcked
		if cwnd != a.CWND() {
			a.setCWND(cwnd)
			a.log.Tracef("[%s] updated cwnd=%d ssthresh=%d acked=%d (CA:%d)",
				a.name, a.CWND(), a.ssthresh, totalBytesAcked, a.congestionController.algorithm())
		}
	}
}
			...
			if !c.acked && !c.abandoned() && c.missIndicator < 3 {
				...
				if c.missIndicator == 3 {
					...
					if !a.inFastRecovery {
						...
						cwnd, ssthresh := a.congestionController.onFastLoss(
							time.Now(), a.CWND(), flightSizeBeforeAck, a.MTU(),
						)
						a.ssthresh = ssthresh
						a.setCWND(cwnd)
...
func (a *Association) processAcknowledgement(
	...
	// Capture FlightSize before this SACK removes cumulatively and selectively
	// acknowledged DATA. RFC 9438's multiplicative decrease is based on the
	// bytes outstanding when the congestion event arrived, not the much smaller
	// post-SACK queue that happens to remain when fast loss is detected below.
	flightSizeBeforeAck := boundedIntToUint32(a.inflightQueue.getNumBytes())
	...
	if sna32LT(a.cumulativeTSNAckPoint, selectiveAckChunk.cumulativeTSNAck) {
		...
		a.onCumulativeTSNAckPointAdvanced(totalBytesAcked, flightSizeBeforeAck)
	}
	...
	return acknowledgementResult{
		...
		flightSizeBeforeAck:     flightSizeBeforeAck,
...
func (a *Association) bundleDataChunksIntoPackets(chunks []*chunkPayloadData) []*packet {
	...
	chunksToSend := []chunk{}
	bytesInPacket := int(commonHeaderSize)
	// Keep upstream Reno's delayed-ACK behavior byte-for-byte compatible. The
	// I-bit is part of YuriRTC's CUBIC recovery policy and is requested only
	// while that controller is at a small window (or its configured floor).
	immediateSACK := a.congestionController != nil &&
		a.congestionController.algorithm() == CongestionControlCUBIC &&
		requestsImmediateSACKAtFloor(a.CWND(), a.MTU(), a.minCwnd)

	for _, chunkPayload := range chunks {
		// RFC 7053 permits the sender to request an immediate SACK when doing
		// so improves throughput. Refresh this at send time—not packetization
		// time—so already-queued DATA and retransmissions react to a later cwnd
		// collapse. Ten MTUs matches the deployed dcSCTP recovery heuristic.
		chunkPayload.immediateSack = immediateSACK
...
func (a *Association) onRetransmissionTimeout(id int, nRtos uint) { //nolint:cyclop
	...
	if id == timerT3RTX { //nolint:nestif
		...
		flightSize := boundedIntToUint32(a.inflightQueue.getNumBytes())
		cwnd, ssthresh := a.congestionController.onTimeout(time.Now(), a.CWND(), flightSize, a.MTU())
		a.ssthresh = ssthresh
		a.setCWND(cwnd)
