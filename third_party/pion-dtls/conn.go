...
type Conn struct {
	...
	writeContextMu       sync.Mutex
	writeContext         context.Context
	writeContextDeadline <-chan struct{}
...
func (c *Conn) compactRawPackets(rawPackets [][]byte) [][]byte {
	// Keep an owned record as-is unless combining it with another ready record
	// actually fits the configured MTU. No delay and no unconditional recopy.
	if len(rawPackets) < 2 {
		return rawPackets
	}
	combined := make([][]byte, 0, len(rawPackets))
	current := rawPackets[0]
	for _, raw := range rawPackets[1:] {
		if len(current)+len(raw) >= c.maximumTransmissionUnit {
