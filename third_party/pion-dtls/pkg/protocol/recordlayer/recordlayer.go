...
func (r *RecordLayer) Marshal() ([]byte, error) {
	// Application data already lives in an owned SCTP packet. Copy it straight
	// into the final record instead of allocating an intermediate payload copy.
	if data, ok := r.Content.(*protocol.ApplicationData); ok {
		r.Header.ContentLen = uint16(len(data.Data))
		r.Header.ContentType = data.ContentType()
		size := r.Header.Size()
		raw := make([]byte, size+len(data.Data))
		if err := r.Header.MarshalTo(raw[:size]); err != nil {
			return nil, err
		}
		copy(raw[size:], data.Data)
		return raw, nil
	}
